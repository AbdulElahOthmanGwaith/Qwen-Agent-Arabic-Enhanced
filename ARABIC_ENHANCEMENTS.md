...
### 3. أداة تحليل المستندات العربية (Arabic Document Parser)

**الملف:** `qwen_agent/tools/arabic_doc_parser.py`

**الوصف:**
أداة متخصصة لاستخراج النصوص من المستندات العربية (PDF, DOCX, TXT) مع تحسين عملية التقسيم (Chunking) لتناسب البحث الذكي (RAG).

**الميزات:**
- استخراج نصوص دقيق من ملفات PDF و Word العربية.
- تنظيف وتطبيع النصوص العربية المستخرجة تلقائياً.
- تقسيم النصوص إلى أجزاء ذكية مع تداخل (Overlap) لضمان عدم فقدان السياق أثناء البحث.

### 4. تحسين البحث الذكي العربي (Arabic RAG Enhancement)

**الملفات المعدلة:** `qwen_agent/memory/memory.py`, `qwen_agent/tools/retrieval.py`, `qwen_agent/agents/assistant.py`

**الوصف:**
تحديث نظام الذاكرة والاسترجاع ليدعم اللغة العربية بشكل أصيل، مما يسمح للوكيل بالعثور على المعلومات داخل المستندات العربية بدقة أعلى بكثير.

**الميزات:**
- استخدام `ArabicDocParser` تلقائياً عند ضبط اللغة على العربية.
- تحسين استعلامات البحث لتتوافق مع النصوص العربية المعالجة.
- دعم كامل للبحث في المستندات العربية عبر واجهة `Assistant`.
...
| `qwen_agent/tools/arabic_doc_parser.py` | أداة تحليل المستندات العربية |
